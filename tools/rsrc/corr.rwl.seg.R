function (rwl, seg.length = 50, bin.floor = 100, n = NULL, prewhiten = TRUE, 
    pcrit = 0.05, biweight = TRUE, method = c("spearman", "pearson", 
        "kendall"), make.plot = TRUE, label.cex = 1, floor.plus1 = FALSE, 
    master = NULL, master.yrs = as.numeric(if (is.null(dim(master))) {
        names(master)
    } else {
        rownames(master)
    }), ...) 
{
    ...
    if (!all(diff(yrs) == 1)) {
        yrs <- min.yr:max.yr
        rwl2 <- matrix(NA_real_, nrow = max.yr - min.yr + 1, 
            ncol = nseries, dimnames = list(as.character(yrs), 
                cnames))
    ...
    if (!is.null(master)) {
        ...
        if (!is.null(master.dim) && length(master.dim) == 2 && 
            ...
            if (!all(diff(master.yrs) == 1)) {
                ...
                master.inc <- matrix(NA_real_, nrow = max.master.yr - 
                  min.master.yr + 1, ncol = master.dim[2], dimnames = list(char.yrs, 
                  colnames(master)))
            ...
            master2 <- rep(NA_real_, max.master.yr - min.master.yr + 
                1)
    ...
    if (is.null(bin.floor) || bin.floor == 0) {
        ...
        min.bin <- ceiling((min.yr - 1)/bin.floor) * bin.floor + 
            1
    }
    else {
    ...
    for (i in seq.series) {
        ...
        for (j in seq_len(nbins)) {
            mask <- yrs %in% seq(from = bins[j, 1], to = bins[j, 
                2])
            if (!any(mask) || any(is.na(series[mask])) || any(is.na(master2[mask]))) {
                bin.cor <- NA
                bin.pval <- NA
            }
            else {
        ...
        tmp <- cor.test(series, master2, method = method2, alternative = "greater")
