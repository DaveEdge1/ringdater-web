function (rwl, series, series.yrs = as.numeric(names(series)), 
    seg.length = 50, bin.floor = 100, n = NULL, prewhiten = TRUE, 
    biweight = TRUE, method = c("spearman", "pearson", "kendall"), 
    pcrit = 0.05, make.plot = TRUE, floor.plus1 = FALSE, ...) 
{
    ...
    if (is.null(bin.floor) || bin.floor == 0) {
        ...
        min.bin <- ceiling((min(series.yrs2) - 1)/bin.floor) * 
            bin.floor + 1
    }
    else {
    ...
    for (j in seq_len(nbins)) {
        mask <- yrs %in% seq(from = bins[j, 1], to = bins[j, 
            2])
        if (!any(mask) || any(is.na(series2[mask])) || any(is.na(master[mask]))) {
            bin.cor <- NA
            bin.pval <- NA
        }
        else {
    ...
    tmp <- cor.test(series2, master, method = method2, alternative = "greater")
    ...
    if (make.plot) {
        ...
        start.below <- seq(from = min.bin - n.below * seg.lag, 
            by = seg.lag, length.out = n.below)
        ticks <- c(start.below, bins[, 1], bins[c(nbins - 1, 
            nbins), 2] + 1)
        nticks <- length(ticks)
        par(mar = c(4, 2, 2, 1) + 0.1, mgp = c(1.25, 0.25, 0), 
            tcl = 0.25)
        sig <- qnorm(1 - pcrit/2)/sqrt(seg.length)
        plot(yrs.tmp, mcor.tmp, type = "l", xlim = c(ticks[1], 
            ticks[nticks]), ylim = range(mcor.tmp, sig, na.rm = TRUE), 
            ylab = gettext("Correlation", domain = "R-dplR"), 
            xlab = gettext("Year", domain = "R-dplR"), sub = gettextf("Segments: length=%d,lag=%d", 
                seg.length, seg.lag, domain = "R-dplR"), axes = FALSE, 
            ...)
    ...
    names(res) <- c("spearman.rho", "p.val", "overall", "bins", 
        "moving.rho")
    res
}
